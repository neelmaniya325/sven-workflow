...
def safe_move_file(src: str, dst: str) -> None:
    """
    Safely move a file, handling cross-device link errors.
    """
    try:
        os.rename(src, dst)
    except OSError as e:
...
def create_anonymized_file(file_path: str, file_extension: str, temp_dir: str) -> str:
    """
    Create anonymized version of the file based on its type.
    """
    base_name = os.path.splitext(os.path.basename(file_path))[0]
    output_path = os.path.join(temp_dir, f"{base_name}_anonymized{file_extension}")
    ...
    if file_extension.lower() == '.docx':
        ...
        output_path = os.path.join(temp_dir, f"{base_name}_anonymized.docx")
        doc.save(output_path)
...
async def upload_and_anonymize_file(file: UploadFile):
    ...
    try:
        ...
        try:
            ...
            # Create anonymized file directly in temp directory
            anonymized_file_path = create_anonymized_file(input_file_path, file_extension, temp_dir)
            
            # Return the anonymized file for download
            return FileResponse(
                path=anonymized_file_path,
                filename=os.path.basename(anonymized_file_path),
                media_type='application/vnd.openxmlformats-officedocument.wordprocessingml.document',
                headers={"Content-Disposition": f"attachment; filename={os.path.basename(anonymized_file_path)}"}
            )
